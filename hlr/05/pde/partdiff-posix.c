...
// params for the pthread computation function
typedef struct { 
  double** Matrix_Out;
  double** Matrix_In;
  int tid; 
...
void *compute_parallel(void *_p) { 
  ...
  for (int i = start; i < end; i++)
  {
      ...
      if (p->options->inf_func == FUNC_FPISIN)
      {
          fpisin_i = p->fpisin * sin(p->pih * (double)i);
      }

      /* over all columns */
      for (int j = 1; j < p->N; j++)
      {
          star = 0.25 * (p->Matrix_In[i-1][j] + p->Matrix_In[i][j-1] + p->Matrix_In[i][j+1] + p->Matrix_In[i+1][j]);

          if (p->options->inf_func == FUNC_FPISIN)
          {
              star += fpisin_i * sin(p->pih * (double)j);
          }

          if (p->options->termination == TERM_PREC || p->term_iteration == 1)
          {
              residuum = p->Matrix_In[i][j] - star;
              residuum = (residuum < 0) ? -residuum : residuum;
              p->maxresiduum = (residuum < p->maxresiduum) ? p->maxresiduum : residuum;
          }

          p->Matrix_Out[i][j] = star;
      }
  }
  return 0; 
}
...
calculate (struct calculation_arguments const* arguments, struct calculation_results *results, struct options const* options)
{
    ...
    for (int i = 0; i < nthreads; i++) { 
      ...
      thread_params[i].N = N;
      thread_params[i].options = options;
      thread_params[i].nthreads = nthreads;
      thread_params[i].fpisin = fpisin;
      thread_params[i].pih = pih;
    } 
	...
	while (term_iteration > 0)
	{
        ...
        for (int i = 0; i < nthreads; i++) { 
          ...
          thread_params[i].Matrix_Out = Matrix_Out; 
          thread_params[i].Matrix_In = Matrix_In; 
