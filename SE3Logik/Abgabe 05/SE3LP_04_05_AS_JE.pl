...
nachfolger(Input,Ergebnis):-
	peano(Input),
% Der Nachfolger ist der Vorgänger ummantelt mit dem Fakt s().
	Ergebnis = s(Input).
...
% Für jeden Aufruf eine Addition mit 1
        Int is X + 1.
...
false.
*/
%Es wird false sowohl für nein, als auch für falsche Typen
%ausgegeben.
...
false.
*/
% Wir stellen keine Veränderung fest (false wenn falsche Eingabe),
