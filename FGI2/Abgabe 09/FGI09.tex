	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=1cm,every path/.style={->}]
				...
				\node (D) [below right = of A] {\((2\omega 0)^T\)};
				\node (E) [below = of D] {\((1\omega 0)^T\)};
				\node (F) [below = of E] {\((0\omega 0)^T\)};
				
				\node (G) [right = of E] {\((0\omega 1)^T\)};
				...
				\path (G) edge [midway, below right] node{b} (F);
				\path (G) edge [midway, above right, bend right=50] node{d} (D);
			\end{tikzpicture} \\ \\
			\begin{tikzpicture}[>=stealth',shorten >=1pt,auto,node distance=1cm,every path/.style={->}]
				%nodes
				\node (A) {\((001)^T\)};
